...
def evaluatePerformance():
    ...
    ** Note that your implementation must follow this API**
    ...
    # Load Data
    filename = 'CIS419/Assignment1/hw1_skeleton/data/SPECTF.dat'
    ...
    #     folds = []    
    #     index = int(len(X)/Trialsfolds)
        # for fold in range(0, Trialsfolds):
        #     # designate a new testing range
        #     Xtest = X[fold * index:(fold + 1) * index,:]
        #     ytest = y[fold * index:(fold + 1) * index,:]
        #     Xtrain = X[:(fold * index),:]
        #     ytrain = y[:(fold * index),:]
        ...
        #     for dataRow in range((fold + 1) * index, len(X)):
        ...
        #     one_Level = one_Level.fit(Xtrain,ytrain)

        #     # train the 3-level decision tree
        #     three_Level = tree.DecisionTreeClassifier(max_depth=3)
        #     three_Level = three_Level.fit(Xtrain,ytrain)

        #     # output predictions on the remaining data
        #     y_pred_tree = clf.predict(Xtest)
        #     y_pred_stump = one_Level.predict(Xtest)
        #     y_pred_dt3 = three_Level.predict(Xtest)
    ...
    three_Level = tree.DecisionTreeClassifier(max_depth=3) 
        ...
        # train_sizes, train_scores, test_scores = learning_curve(clf, X, y, cv=10, scoring='accuracy', n_jobs=-1, train_sizes=np.linspace(0.01, 1.0, 50))
        ...
        # axes=axes[:, 0]
        # axes[0].plot(train_sizes, train_mean, '--', color="#111111",  label="Training score")
        # axes[0].plot(train_sizes, test_mean, color="#111111", label="Cross-validation score")
        # axes[0].fill_between(train_sizes, train_mean - train_std, train_mean + train_std, color="#DDDDDD")
        # axes[0].fill_between(train_sizes, test_mean - test_std, test_mean + test_std, color="#DDDDDD")
