...
class LinearRegression:
    ...
    def gradientDescent(self, X, y, theta):
        ...
        for i in range(self.n_iter):
            ...
            factor = [0] * thetaDimensions
            # print("BEFORE", theta.shape)
            # print("factor", thetaDimensions)
            ...
            for j in range(0,n):
                for thetaDimension in range(0,thetaDimensions):
                    factor[thetaDimension] += (theta.getT()*X[j,:].getT() - y[j])*X[j,thetaDimension]
            for thetaDimension in range(0,thetaDimensions):
                theta[thetaDimension] = theta[thetaDimension] - factor[thetaDimension]*(self.alpha/n)
            # print("AFTER",theta)
        return theta
